...
export default function AuthForm({type, setIsLoggedIn}: {type: 'signup' | 'login' | 'verify', setIsLoggedIn: (arg0: boolean) => any}) {
  ...
  const handleVerify = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    user.confirmRegistration(password, false, (err, result) => {
      if (err) {
        setError(err.message);
      } else {
  ...
  const submitHandler = type === 'signup' ? handleSignup : type === 'login' ? handleLogin : handleVerify
  const buttonLabel = type === 'signup' ? 'Create account' : type === 'login' ? 'Sign in' : 'Submit'


  return <div id='auth-page-wrapper'>
        ...
        <div id='inside-auth-form-wrapper'>
          <div id='auth-input-wrapper'>
            ...
            { type !== 'verify' && (type === 'signup' ?
              <span>Already have an account? <Link to={urlStart+ 'login'}>Sign in</Link></span>
              :
              <span>New to Boardcast? <Link to={urlStart + 'signup'}>Sign up</Link></span>
            )}
          </div>
        </div>
      </form>
