...
export default function EditAgendaItem({
  agendaItem, orderNum, updateAgendaItem, upload, removeAgendaItem,
}: {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const resp = await $.ajax(updateAgendaItemURL, {
      ...
      headers,
      data: JSON.stringify({
        ...agendaItem, title, timeEstimate, description, orderNum, meetingID, attachments,
      } as AgendaItem),
    });
  ...
  return (
    <form id="agenda-item-form" onSubmit={handleSubmit}>
      <input type="text" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} />
      <span>(</span>
      <input type="number" placeholder="minutes" value={timeEstimate} onChange={(e) => setTimeEstimate(Number(e.target.value))} />
      <span>min)</span>
      <input type="file" onChange={handleFileChange} />
      <textarea placeholder="description" cols={30} rows={10} value={description} onChange={(e) => setDescription(e.target.value)} />
      <button type="submit">Submit</button>
      {agendaItem && <button onClick={handleDelete} type="button">Delete</button>}
    </form>
  );
}
