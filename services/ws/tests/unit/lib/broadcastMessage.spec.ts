...
describe('broadcastMessage', () => {
  ...
  it('sends the message and email to all connections in the database matching the meetingID', async () => {
    ...
    const testMessage = {
      wsConnectionID: meetingConnections[0].wsConnectionID,
      meetingID,
      text: testText,
      email: testEmail,
    ...
    meetingConnections.forEach((connection) => {
      expect(mockSend).toHaveBeenCalledWith(expect.objectContaining({
        input: {
          ConnectionId: connection.wsConnectionID,
          Data: testMessageEncoded,
        },
      }));
    });
  });
  ...
  afterAll(() => {
    const deleteConnection = (connection: any) => removeConnection(connection.wsConnectionID);
    const removalPromises = meetingConnections.map(deleteConnection);
